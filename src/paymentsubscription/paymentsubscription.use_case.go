...
// UseCaseHandler provides a convenient interface for PaymentSubscription use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the PaymentSubscription data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (PaymentSubscription, error) {
	res := PaymentSubscription{}

	// check permission
	err := u.Ctx.ValidatePermission("payment_subscriptions.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of PaymentSubscription data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("payment_subscriptions.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &PaymentSubscription{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &PaymentSubscription{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data PaymentSubscription with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("payment_subscriptions.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(PaymentSubscription{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the PaymentSubscription data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("payment_subscriptions.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the PaymentSubscription data for the specified ID with specified parameters.
...
// DeleteByID deletes the PaymentSubscription data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("payment_subscriptions.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update PaymentSubscription data.
func (u *UseCaseHandler) setDefaultValue(old PaymentSubscription) error {
	if !old.ID.Valid {
