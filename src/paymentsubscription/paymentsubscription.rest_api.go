...
// RESTAPIHandler provides a convenient interface for PaymentSubscription REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the PaymentSubscription REST API handler.
...
// GetByID is the REST API handler for `GET /api/payment_subscriptions/{id}`.
...
// Get is the REST API handler for `GET /api/payment_subscriptions`.
...
// Create is the REST API handler for `POST /api/payment_subscriptions`.
...
// UpdateByID is the REST API handler for `PUT /api/payment_subscriptions/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/payment_subscriptions/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/payment_subscriptions/{id}`.
func (r *RESTAPIHandler) DeleteByID(c *fiber.Ctx) error {
	...
	res := map[string]any{
		...
		"message": r.UseCase.Ctx.Trans("deleted", map[string]string{
			"payment_subscriptions": p.EndPoint(),
			"id":                    c.Params("id"),
		}),
	}
	return c.JSON(res)
