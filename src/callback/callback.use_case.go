...
// UseCaseHandler provides a convenient interface for Callback use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Callback data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Callback, error) {
	res := Callback{}

	// check permission
	err := u.Ctx.ValidatePermission("callback.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Callback data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("callback.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Callback{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &Callback{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Callback with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("callback.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(Callback{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the Callback data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("callback.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Callback data for the specified ID with specified parameters.
...
// DeleteByID deletes the Callback data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("callback.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Callback data.
func (u *UseCaseHandler) setDefaultValue(old Callback) error {
	if !old.ID.Valid {
