...
// RESTAPIHandler provides a convenient interface for Callback REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Callback REST API handler.
...
// GetByID is the REST API handler for `GET /api/callback/{id}`.
...
// Get is the REST API handler for `GET /api/callback`.
...
// Create is the REST API handler for `POST /api/callback`.
...
// UpdateByID is the REST API handler for `PUT /api/callback/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/callback/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/callback/{id}`.
