...
// UseCaseHandler provides a convenient interface for Contact use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Contact data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Contact, error) {
	res := Contact{}

	// check permission
	err := u.Ctx.ValidatePermission("contacts.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Contact data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("contacts.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Contact{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &Contact{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Contact with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("contacts.create")
	if err != nil {
		return err
	}
...
// UpdateByID updates the Contact data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("contacts.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Contact data for the specified ID with specified parameters.
...
// DeleteByID deletes the Contact data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("contacts.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Contact data.
func (u *UseCaseHandler) setDefaultValue(old Contact) error {
	if !old.ID.Valid {
