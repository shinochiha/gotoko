...
// RESTAPIHandler provides a convenient interface for Contact REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Contact REST API handler.
...
// GetByID is the REST API handler for `GET /api/contacts/{id}`.
...
// Get is the REST API handler for `GET /api/contacts`.
...
// Create is the REST API handler for `POST /api/contacts`.
...
// UpdateByID is the REST API handler for `PUT /api/contacts/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/contacts/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/contacts/{id}`.
