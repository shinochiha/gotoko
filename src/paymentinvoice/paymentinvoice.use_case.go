...
// UseCaseHandler provides a convenient interface for PaymentInvoice use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the PaymentInvoice data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (PaymentInvoice, error) {
	res := PaymentInvoice{}

	// check permission
	err := u.Ctx.ValidatePermission("payment_invoices.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of PaymentInvoice data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("payment_invoices.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &PaymentInvoice{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &PaymentInvoice{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data PaymentInvoice with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("payment_invoices.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(PaymentInvoice{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the PaymentInvoice data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("payment_invoices.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the PaymentInvoice data for the specified ID with specified parameters.
...
// DeleteByID deletes the PaymentInvoice data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("payment_invoices.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update PaymentInvoice data.
func (u *UseCaseHandler) setDefaultValue(old PaymentInvoice) error {
	if !old.ID.Valid {
