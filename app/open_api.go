...
func (o *openAPIUtil) Configure() *openAPIUtil {
	...
	o.Info.Title = "Gotoko"
	o.Info.Description = "The Gotoko allows you to perform all the operations that you do with our applications. " +
		"Gotoko is built using REST principles which ensures predictable URLs, uses standard HTTP response codes, " +
		`authentication, and verbs that makes writing applications easy.

## Query Params

Gotoko support a common way for pagination, sorting, filtering, searching and other using URL query params on ` + "`GET`" + ` method.
